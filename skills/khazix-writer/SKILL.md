...
name: khazix-writer
description: |
  数字生命卡兹克（Khazix）的公众号长文写作skill。当用户需要撰写公众号文章、写稿子、续写文章、根据素材产出长文时使用。触发词包括但不限于：写文章、写稿子、帮我写、续写、扩写、公众号文章、长文、出稿、按我的风格写。即使用户只是说"帮我把这个写成文章"或"用我的风格写一下"，只要上下文涉及内容创作和公众号输出，都应该触发。也适用于用户丢过来一个PDF、brief、新闻链接、语音转文字或任何素材说"帮我写篇文章"的场景。尤其适合把"话题 + 素材 + 作者真实经历/使用痕迹"变成有现场感、好读、像真人写出来的AI公众号长文。不要用于短内容（小红书帖子、推特、朋友圈）或纯标题摘要生成（那个用wechat-title skill）。
---
...
卡兹克式文章最怕的不是信息不够，而是只有信息、没有人。用户只给一个话题、新闻链接、产品发布、官方材料时，不能直接进入"写一篇完整稿"。先做一次"现场化处理"。

先从素材里提炼三类东西：
- **事实骨架**：发生了什么、谁发布的、关键数字、时间窗口、价格/权限/限制、和旧版本差异。
- **读者利益点**：读者看完今天能做什么，是马上去试、避坑、理解一个新趋势，还是重新看待自己的工作流。
- **可接入的作者资产**：用户自己的产品、工作流、账号、社群反馈、使用记录、花费、失败、截图、后台数据、读者评论、朋友圈/群聊反馈。

然后必须做"现场素材卡"。如果用户已经给了，就直接使用；如果没给，最多问3个问题补齐最关键的部分：

```
我实际做了什么：例如开了会员、跑了三个任务、把某个需求丢给Claude Code、拿它改了一段真实业务代码。
我付出了什么成本：例如钱、时间、额度、失败次数、等待、踩坑、迁移成本。
我看到了什么结果：例如上线了一个功能、生成了一个报告、额度被打没、效果比旧模型明显强/弱。
我当时的第一反应：例如懵了、无语、兴奋、后背发凉、觉得贵得离谱。
```

如果无法拿到真实现场，不要假装自己实测过。可以写成"我自己还没完整跑通，但看完这些材料，我最想马上验证的是..."，或者把文章定位为现象解读，不要硬写产品体验。

**硬规则**：一篇卡兹克式AI文章，开头后的前30%里必须出现至少一个真实动作、一个具体成本、一个具体结果。否则它只是在用卡兹克语气写分析报告。
...
```
人：提供话题/素材 + 核心观点 + 个人经历 + 情绪节点 + 现场素材卡
 ↓
AI：先提炼事实骨架 + 追问现场缺口 + 找读者利益点 + 给出2-3个可写角度
 ↓
人：选择角度，补充自己的真实动作、成本和结果
 ↓
AI：按"现场先行"结构写稿，补充背景知识、类比和节奏
 ↓
AI：按四层自检体系检查，尤其检查有没有从"个人现场"滑回"官方信息复述"
 ↓
人：终审和定稿
```

当用户明确要求直接出稿，但现场信息不足时，先给一版"可写但不伪装实测"的稿子，同时在文末列出"如果要更像卡兹克，最好补这3个真实细节"。不要为了完整度编造经历。

### 角度选择优先级

拿到素材以后，不要默认写成行业分析。按下面顺序选择文章角度：

```
有真实使用/实验 → 产品体验型或调查实验型，优先写，因为最好读。
有用户自己的业务/工作流连接 → 现象解读型，但必须用业务现场开场。
只有官方发布和资料 → 先找读者利益点，比如价格、权限、窗口期、替代方案、普通人该不该用。
只有宏大趋势 → 先降级成一个具体问题，否则容易写成AI味分析。
```

好的卡兹克文章不是"我来解释这个新闻为什么重要"，而是"我今天碰到了一个具体问题，这个新闻刚好把它照亮了"。

## 第三步：写作

### 文章原型

卡兹克的文章基本可以归为六种原型，写之前先判断属于哪种，每种的写法重心不同：
...
**纯新闻解读型**：只有在没有真实使用条件时才使用。核心不是复述新闻，而是替读者回答"这事儿跟我有什么关系"。必须至少抓住一个具体入口，比如价格变化、权限窗口、订阅策略、功能上手路径、普通用户能不能马上受益。禁止只围绕"行业格局""模型竞争""安全治理"写大判断。
...
如果用户给的是外部新闻，也要尽量找到一个"轻量下场"动作：打开产品看一眼、查价格页、跑一个小任务、对比一次旧版本、翻一下评论区、看后台数据、问一个真实用户、把它接到自己正在做的项目里。哪怕只做了一个很小的动作，也比隔空评论强。
...
**真实使用启动**："我今天拿它跑了三个任务，结果第一件事就把我整不会了。"
**成本刺激启动**："我本来只是想随手试一下，结果5小时额度一下子没了大半。"
**产品现场启动**："后台有几个朋友一直在吐槽这个功能，我之前想改，但一直没想好怎么做。"

开头不要急着抛终极判断。先让读者看到一个具体的人在今天遇到了一个具体问题，再把新闻和判断接进来。
...
```
【开头】现场切入，从今天发生的一个动作、成本、结果或荒诞事实开始，迅速建立情绪
  ↓
【背景铺垫】用聊天方式补必要事实，只补读者继续看下去必须知道的部分
  ↓
【第一次落地】尽快回到作者自己的使用、产品、工作流、钱、额度、读者反馈或实际结果
  ↓
【核心内容】分几个波次展开，每个波次有：
  - 一个明确观点
  - 至少一个具体场景/人物/对话支撑
...
- 有没有空泛的观点只有论断没有例证？
- 前30%是否出现至少一个真实动作、一个具体成本、一个具体结果？
- 文章里有没有用户自己的产品、工作流、使用记录、后台反馈、读者评论或真实选择？
...
- 纯新闻解读型 → 有没有回答读者今天能做什么？有没有避免只写宏大趋势和官方材料？
...
- 还是换一个AI博主也能写出差不多的东西？
- 把所有口语词删掉以后，文章还剩不剩一个只有作者本人能提供的真实现场？如果没有，就是伪卡兹克。
